...
for(var id in CKEDITOR.instances) {
		if (CKEDITOR.instances[id].config.wiriscontextpath) _wrs_cont_contextPath = br.baseUrl + CKEDITOR.instances[id].config.wiriscontextpath
}
		...
		function whenDocReady() {
			if (window.wrs_initParse && typeof _wrs_conf_configuration_loaded != 'undefined' && lastDataSet != null) { // WIRIS plugin core.js and configuration loaded properly
				...
				editor.on('setData', function (e) {
						var dataValue = e.data.dataValue;
					  if(dataValue === null)dataValue = '';
						e.data.dataValue= wrs_initParse(dataValue);
				});
				...
				// I can't understand why we call setdata method. But this call onchange
...
function wrs_int_openNewFormulaEditor(element, language, isIframe) {
	...
	_wrs_int_window.onunload = function(){
...
function wrs_int_doubleClickHandler(editor, target, isIframe, element, event) {
	if (element.nodeName.toLowerCase() == 'img') {
		...
		if (wrs_containsClass(element, _wrs_conf_CASClassName)) {
			_wrs_temporalImage = element;
