(function(c) {
    ...
    "4": function(d, c, e, b) {
        ...
        for (var g = "string,number,array,object,date,function,regexp".split(","), f = g.length; f--;) {
            h[g[f]] = l();
        }
        c.exports = h;
    },
    ...
    this.RokBox.Class = new Class({
        ...
        attach: function() {
            ...
            var j = document.retrieve(this.options.data + ":open", function(o, n) {
                ...
                h = document.retrieve(this.options.data + ":fitscreen", function(o, n) {
                    if (o && o.rightClick) {
                        return true;
                    }
                    this.fitscreen(n, o);
                }.bind(this)),
                k = document.retrieve(this.options.data + ":unfitscreen", function(o, n) {
                    if (o && o.rightClick) {
                        return true;
                    }
                    this.unfitscreen(n, o);
                }.bind(this)),
                i = document.retrieve(this.options.data + ":toggleFitscreen", function(o, n) {
                    if (o && o.rightClick) {
                        return true;
                    }
                    this[(this.isFitting) ? "unfitscreen" : "fitscreen"](n);
                }.bind(this)),
                g = document.retrieve(this.options.data + ":navigation", function(p, n) {
                    var o;
                    if (p && p.rightClick) {
                        return true;
                    }
        ...
        load: function(p, j) {
            ...
            var k = p.get("href"),
                l = this.media.format(k),
                v = this.media.getType(k),
                i = this.media.getAspect(k),
                n, t, o = {},
                m, y, u = true,
                g;
            window.removeEvent("resize", this.bound.resize);
            ...
            switch (v) {
                ...
                case "video":
                    ...
                    break;
                case "swf":
                    var x = Math.min(window.getSize().x - 100, i.w),
                        q = x * i.h / i.w;
                    if (y) {
                        x = y[0];
                        q = y[1];
                    }
                    var s = new Element("embed", {
                        src: l,
        ...
        setNavigation: function(h) {
            var o = h.get("data-rokbox-album"),
                n = document.getElements("[data-rokboxprevious], [data-rokboxnext]"),
                i = document.getElements("[data-rokboxprevious]"),
                m = document.getElements("[data-rokboxnext]");
            if (!o) {
                n.setStyle("display", "none");
                return this;
            }
            var g = document.getElements("[data-rokbox-album=" + o + "]"),
                l = g.indexOf(h),
                k = g[l - 1],
                j = g[l + 1];
            if (l == -1 || g.length == 1) {
        ...
        _build: function() {
            ...
            var h = ["outer", "row", "inner", "container"],
                g = ["loader", "content", "controls"],
                i = {
                    x: "close",
                    p: "previous",
                    n: "next",
        ...
        _loadImage: function(n, p) {
            var o = this.containerContent.getSize(),
                t, k, h = {};
            this.setType(p.type);
            if (f) {
            ...
            if (p.videoSize) {
                t = {
                    x: p.videoSize.width || t.x,
                    y: p.videoSize.height || t.y
                };
            }
            k = this.container.getComputedSize({
            ...
            var r = window.getSize(),
                j = this.containerContent.getSize();
            if (k >= r.y) {
                var m = this.container.getStyle("margin-bottom").toInt(),
                    s = document.getElement("[data-rokboxcaption]"),
                    q = s ? s.getSize().y : 0,
                    g = Math.round(r.y * t.y / k) - q - m,
                    i = Math.round(t.x * g / t.y);
                if (!this.isFitting) {
                    t.x = i;
                    t.y = g;
                }
        ...
        _loadIframe: function(l, n) {
            ...
            if (i >= p.y) {
                var k = this.container.getStyle("margin-bottom").toInt(),
                    ...
                    o = q ? q.getSize().y : 0,
                    g = Math.round(p.y * r.y / i) - o - k,
                    h = Math.round(r.x * g / r.y);
                if (!this.isFitting) {
                    r.x = h;
                    r.y = g;
                }
        ...
        _resizeObject: function(i, l) {
            var k = this.container.getStyle("margin-bottom").toInt(),
                h = document.getElement("[data-rokboxcaption]"),
                j = h ? h.getSize().y : 0,
                g = window.getSize(),
                m = this.containerContent.getSize(),
        ...
        _openAndFixJump: function() {
            var h = afterSize = document.body.scrollWidth,
                g = 0;
